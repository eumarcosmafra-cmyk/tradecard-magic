...
const SHOPIFY_STORE_PERMANENT_DOMAIN = '30q0dh-rj.myshopify.com';
const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
const SHOPIFY_STOREFRONT_TOKEN = '5431bcedc66d547a18d9fddfeea6f0e1';
...
export async function storefrontApiRequest(query: string, variables: Record<string, unknown> = {}) {
  ...
  if (data.errors) {
    throw new Error(`Erro Shopify: ${data.errors.map((e: { message: string }) => e.message).join(', ')}`);
  }
