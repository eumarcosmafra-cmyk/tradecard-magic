...
export const GoldenBallers = () => (
  <section className="mt-20 -mx-4 md:-mx-8 lg:-mx-16 px-4 md:px-8 lg:px-16 py-16 bg-secondary rounded-3xl">
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
      {/* Left: card image */}
      <div className="bg-secondary-foreground/10 border-2 border-primary/30 rounded-2xl p-8 flex items-center justify-center">
        <img
      ...
      <div className="space-y-6">
        ...
        <h2 className="font-display text-4xl md:text-5xl tracking-wider uppercase text-secondary-foreground">Os Golden Ballers</h2>
        <p className="text-secondary-foreground/70 font-body leading-relaxed">
          Pela <strong className="text-secondary-foreground">primeira vez</strong> numa coleção FIFA World Cup™! Os 9 maiores craques do planeta em cards premium dourados — <strong className="text-secondary-foreground">100 pontos em TODOS os valores de jogo</strong> para sempre no Adrenalyn XL™. Cada envelope esconde a chance de um deles.
        </p>
        ...
        <div className="grid grid-cols-3 gap-3">
          {goldenBallers.map((player) => (
            <div
              key={player.name}
              className="bg-secondary-foreground/10 border border-secondary-foreground/20 rounded-xl p-3 flex items-center gap-3 hover:border-primary/50 transition-colors"
            >
              <img src={player.flag} alt={player.country} className="w-7 h-5 object-cover rounded-sm flex-shrink-0" />
              <div className="min-w-0">
                <p className="font-display text-sm tracking-wider uppercase truncate text-secondary-foreground">{player.name}</p>
                <p className="text-[10px] text-secondary-foreground/50 font-body truncate">{player.country}</p>
              </div>
            </div>
          ))}
        </div>
